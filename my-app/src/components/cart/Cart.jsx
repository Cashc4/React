...
const onAddOverlay = async(obj) => {
    try{
        ...
        {
            axios.delete(`https://63819cad9842ca8d3c965d38.mockapi.io/cart/${findOverlay.id}`)
           props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))

           }
    else{
       const{data} = await axios.post('https://63819cad9842ca8d3c965d38.mockapi.io/cart', obj)
...
const onAddFav = async (obj)=>{
    ...
    try{
        const findFavorites = props.favorites.find(objFav => objFav.myId === obj.myId)
         if(findFavorites){
        axios.delete(`https://63819cad9842ca8d3c965d38.mockapi.io/favorites/${findFavorites.id}`)
        props.setFavorites((over) => over.filter(item => item.myId !== obj.myId))
    }
    else{
      const {data} = await  axios.post('https://63819cad9842ca8d3c965d38.mockapi.io/favorites', obj)
